...
export class UIManager {
    constructor(game) {
        ...
        this.fullscreenIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"></path></svg>`;
        this.exitFullscreenIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z"></path></svg>`;
    }
     ...
     */
    init() {
        if (!this.startMenu || !this.startButton || !this.controlsGuide || !this.muteButton || !this.fullscreenButton || !this.touchControls || !this.touchToggleCheckbox) {
    ...
    setupFullscreenButton() {
        this.updateFullscreenIcon(); // Set initial icon
    ...
    updateFullscreenIcon() {
        if (document.fullscreenElement) {
            this.fullscreenButton.innerHTML = this.exitFullscreenIcon;
        } else {
    ...
    async toggleFullscreen() {
        const targetElement = this.game.gameContainer;
    
        if (!document.fullscreenElement) {
            try {
                await targetElement.requestFullscreen();
                // On mobile, lock orientation after entering fullscreen
                if (this.isTouchDevice && screen.orientation && typeof screen.orientation.lock === 'function') {
